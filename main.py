...
SECRET_CODE_1 = "ZETA-7"
SECRET_CODE_2 = "OMEGA-9"
# ──────────────────────────────────────────────────────────────────────────────
...
def header():
    ...
    print()
    divider("═", 60)
    instant_print("    TERMINALE PERSONALE // RICERCATORE INDIPENDENTE E.V.", DIM_GREEN)
...
def funzione_1():
    ...
    segnali = [
        (
            ...
            "18m 44s",
            "PARZIALE",
            "Prima ricezione. Non dormivo da 4 giorni.",
        ),
        (
            "S-002",
            "07.11.2014",
            ...
            "02m 11s",
            "RUMORE",
            "Interferenza. O forse no. Ci penso ancora.",
        ),
        (
            "S-003",
            "29.06.2016",
            ...
            "41m 03s",
            "CONFERMATO",
            "Risposta. Ne sono CERTO. Era una risposta.",
        ),
        (
            "S-004",
            "12.02.2018",
            ...
            "07m 58s",
            "PARZIALE",
            "Volevano qualcosa. Ho capito dopo.",
        ),
        (
            "S-005",
            "03.09.2019",
            ...
            "1h 12m",
            "CONFERMATO",
            "Il messaggio era chiaro. TRIBUTO. PONTE.",
        ),
        (
            "S-006",
            "21.12.2021",
            ...
            "3h 00m",
            "CONFERMATO",
            "Solstizio. Non è un caso. Non è MAI un caso.",
        ),
        (
            "S-007",
            "??/??/????",
            ...
            "???",
            "IN CORSO",
            "Stanno aspettando. Il tributo non era giusto.",
        ),
    ]
    ...
    for sid, data, freq, dur, stato, nota in segnali:
        color = (
            BRIGHT_GREEN
            if stato == "CONFERMATO"
            else (YELLOW if stato == "IN CORSO" else DIM_GREEN)
        )
    ...
    slow_print(
        "  È la stessa della risonanza della camera cranica umana.", 0.02, DIM_GREEN
    )
...
def funzione_2():
    ...
    print()
    divider("═", 60)
    slow_print("  >> DIARIO DI RICERCA // LOG ESPERIMENTI - E.V.", 0.04, BRIGHT_GREEN)
    divider("═", 60)
    print()
    ...
    voci = [
        ...
        (
            ...
            "  Risultato: negativo. La combinazione creava interferenza invece di amplificare.\n"
    ...
    print()
    slow_print(
        "  >> [ULTIMA VOCE NON DATATA - scritta a mano, scansionata]:", 0.03, YELLOW
    )
    slow_print(
        '  "Questa volta funzionerà. Devono farlo funzionare. Ho dato troppo."',
        0.02,
        DIM_GREEN,
    )
    print()
...
# ─── FUNZIONE 3: Protocollo Preparazione Tributo ─────────────────────────────
...
def funzione_3():
    ...
    slow_print("  >> DOCUMENTO OPERATIVO - PROGETTO PONTE - E.V.", 0.03, YELLOW)
    divider("═", 60)
    print()

    slow_print("  PREMESSA:", 0.03, BRIGHT_GREEN)
    slow_print(
        "  Il contatto extradimensionale richiede un medium biologico preparato.",
        0.02,
        DIM_GREEN,
    )
    slow_print(
        "  Il soggetto deve essere 'sintonizzato' prima della finestra di contatto.",
        0.02,
        DIM_GREEN,
    )
    slow_print(
        "  La preparazione richiede precisione. Gli errori passati lo dimostrano.",
        0.02,
        DIM_GREEN,
    )
    print()
    ...
    fasi = [
        (
            ...
            [
                ...
                "Preferibile: nessun familiare nelle vicinanze immediati.",
        ...
        (
            ...
            [
                ...
                "Durata: Poche ore. Prece Versioni precedenti usavano 96h: controindicato. Vicinanza a condizione naturale consigliata.",
            ],
        ),
        (
            "FASE 3",
            "CALIBRAZIONE FREQUENZIALE",
            [
    ...
    print()
    slow_print(
        "  >> STATO ATTUALE: FASE 4 // FINESTRA DI CONTATTO APERTA", 0.03, YELLOW
    )
...
def funzione_4():
    ...
    slow_print(
        "  Questa è la sessione attiva collegata ai soggetti in preparazione.",
        0.02,
        DIM_GREEN,
    )
    slow_print(
        "  Il canale è bidirezionale. Quello che viene trasmesso, viene ricevuto.",
        0.02,
        DIM_GREEN,
    )
    ...
    for msg in messaggi1:
        time.sleep(random.uniform(0.6, 1.4))
        slow_print(
            msg,
            0.025,
            GREEN
            if "RICONOSCIUTA" not in msg and "INSUFFICIENTE" not in msg
            else YELLOW,
        )
...
def funzione_5():
    ...
    print()
    slow_print(
        "  Questa procedura interrompe il protocollo di preparazione.", 0.03, DIM_GREEN
    )
    slow_print(
        "  Se confermata, rilascia i soggetti e disattiva le apparecchiature.",
        0.03,
        DIM_GREEN,
    )
    print()
    slow_print("  >> Inserire la sequenza di interruzione.", 0.03, YELLOW)
    print()
    divider("─", 60, DIM_GREEN)
    print()

    sys.stdout.write(f"{BRIGHT_GREEN}  >> CODICE INTERRUZIONE PRIMARIO: {RESET}")
    sys.stdout.flush()
    code1 = input().strip().upper()

    if code1 != SECRET_CODE_1:
        print()
        slow_print("  >> CODICE NON VALIDO.", 0.04, RED)
        slow_print("  >> INTERRUZIONE RIFIUTATA. SPERIMENTAZIONE CONTINUA.", 0.04, RED)
        slow_print("  >> ANOMALIA REGISTRATA NEL LOG.", 0.04, RED)
        matrix_rain(1.0)
        print()
        divider("─", 60, DIM_GREEN)
        input(f"{DIM_GREEN}  Premi INVIO per tornare al menu...{RESET}")
    ...
    print()
    slow_print("  >> PRIMO CODICE ACCETTATO.", 0.04, GREEN)
    loading_bar("  Verifica autorizzazione livello 1      ", 0.9)
    print()

    sys.stdout.write(f"{BRIGHT_GREEN}  >> CODICE INTERRUZIONE SECONDARIO: {RESET}")
    sys.stdout.flush()
    code2 = input().strip().upper()

    if code2 != SECRET_CODE_2:
        print()
        slow_print("  >> SECONDO CODICE NON VALIDO.", 0.04, RED)
        slow_print("  >> SEQUENZA INTERROTTA. SPERIMENTAZIONE CONTINUA.", 0.04, RED)
        slow_print("  >> DOPPIA ANOMALIA REGISTRATA. AVVISO INVIATO A E.V.", 0.04, RED)
        matrix_rain(1.0)
        print()
        divider("─", 60, DIM_GREEN)
        input(f"{DIM_GREEN}  Premi INVIO per tornare al menu...{RESET}")
    ...
    # ── SUCCESSO ──
    print()
    loading_bar("  Verifica secondo codice                ", 0.8)
    loading_bar("  Autenticazione completata              ", 1.0)
    ...
    steps = [
        "  Spegnimento emettitore 432.7 Hz...",
        "  Spegnimento risonatore Schumann...",
        "  Espulsione disco di sicurezza...",
    ]
    for step in steps:
        slow_print(step, 0.025, GREEN)
        time.sleep(0.6)
